...
//        String query = "INSERT INTO USERS(EMAIL, PASS, ROLE) VALUES('"+ email +"','"+pass+"','"+role+"')";
        System.out.println(email + " first " + pass);
        String query = "Select PASSWORD, ROLE from USERS ";
...
//            response.sendRedirect("/");
    ...
    public boolean reg(HttpServletRequest request, HttpServletResponse response, String where) {
        HttpSession session = request.getSession(true);
...
//        String query = "INSERT INTO USERS(EMAIL, PASS, ROLE) VALUES('"+ email +"','"+pass+"','"+role+"')";
        System.out.println(email + " first " + pass);
        try {
            CallableStatement conn = dbWorker.connection.prepareCall("{call registernewusers(?, ?, ?, ?, ?, ?)}");
            conn.setString(1, pass);
            conn.setString(2, role);
            conn.setString(3, email);
            conn.setString(4, group);
            conn.setString(5, recordBook);

            conn.registerOutParameter(6, Types.INTEGER);

            ResultSet resultset = conn.executeQuery();
            boolean outter_param = (int) conn.getObject(6) != 1;
